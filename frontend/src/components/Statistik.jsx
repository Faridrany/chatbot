...
function CMCell({ value, isTP }) {
  ...
  return (
    <td className={`text-center p-3 text-sm border ${bg}`}>{value}</td>
  );
}
...
export default function Statistik({ onLogout }) {
  ...
  if (loading) return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1"><Header /><main className="p-8 text-gray-500">Memuat statistik model...</main></div>
    </div>
  );
  ...
  if (error || !data) return (
    <div className="flex min-h-screen bg-gray-50">
      <Sidebar onLogout={onLogout} />
      <div className="flex-1"><Header /><main className="p-8 text-red-500">Gagal memuat data: {error}</main></div>
    </div>
  );
        ...
        <main className="p-8 space-y-8">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Evaluasi performa model Random Forest dengan TF-IDF
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center py-2">
                  ...
                  <span className="font-semibold text-green-700">TF-IDF</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center py-2">
                  <span className="text-sm text-gray-500">Akurasi</span>
                  <span className="font-bold text-green-700 text-lg">{(akurasi * 100).toFixed(0)}%</span>
                </div>
              </div>
            </div>
          </div>
              ...
              <table className="w-full border-collapse text-sm">
                ...
                <tbody>
                  {CATEGORIES.map((actual) => {
                    ...
                    return (
                      <tr key={actual}>
                        <td className="p-3 border font-semibold text-gray-700 bg-gray-50">
                          {CAT_LABEL[actual]}
                        </td>
                        {CATEGORIES.map((predicted) => (
                          <CMCell
                            key={predicted}
                            value={confusionMatrix[actual]?.[predicted] || 0}
                            isTP={actual === predicted}
                          />
                        ))}
                        <td className="p-3 text-center border text-gray-500 font-semibold bg-gray-50">
                          {rowTotal}
                        </td>
                      </tr>
                    );
              ...
              <table className="w-full text-sm border-collapse">
                ...
                <tbody>
                  {CATEGORIES.map((cat) => {
                    ...
                    return (
                      <tr key={cat} className="border-b hover:bg-gray-50">
                        ...
                        <td className="p-3 text-center border font-semibold text-green-700">
                  ...
                  <tr className="bg-green-50 font-semibold">
                    ...
                    <td className="p-3 text-center border text-gray-500" colSpan={4}>—</td>
                  </tr>
                </tbody>
              </table>
            </div>
